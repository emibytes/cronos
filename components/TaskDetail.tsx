    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      >
        <div className="flex-1 flex flex-col min-h-0 border-r border-gray-100 dark:border-gray-800">
          <div className="p-6 border-b border-gray-50 dark:border-gray-800 flex justify-between items-start bg-gray-50/50 dark:bg-white/5">
          ...
          <div className="p-6 border-t border-gray-100 dark:border-gray-800 bg-gray-50/30 dark:bg-black/10 flex justify-between items-center">
             <button 
              onClick={() => onDelete(task.id)}
              className="flex items-center gap-2 text-sm text-red-500 hover:text-red-600 font-bold px-4 py-2 rounded-xl transition-colors"
            >
              <Trash2 size={16} />
              ELIMINAR TAREA
            </button>
            <div className="flex gap-3">
              {(isEditing || isChanged) && (
                <button 
                  onClick={handleSave}
                  className="flex items-center gap-2 px-6 py-2.5 bg-emibytes-primary text-white rounded-xl font-black shadow-lg shadow-emibytes-primary/20 hover:scale-[1.02] transition-all uppercase text-sm"
                >
                  <Save size={18} />
                  Guardar Cambios
                </button>
        ...
        <div className="w-full md:w-72 bg-gray-50 dark:bg-black/20 p-6 space-y-8 overflow-y-auto">
          <section className="space-y-4 text-center pb-4">
             {!logoError ? (
               <img src="input_file_2.png" alt="Brand" className="w-16 h-16 mx-auto opacity-50 grayscale hover:grayscale-0 transition-all cursor-help" title="emibytes Task Engine" />
             ) : (
